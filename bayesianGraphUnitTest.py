...
class TestGraphMethods(unittest.TestCase):
	...
	##checks that the network get_data() returns the appropriate data points
	##from the JSON generated by the superclass
    #@unittest.skip
	...
	#the general format is a list composed of elements
	#[statename(str), [choicename(str), prob(float)]
    #@unittest.skip
    def test_data_accuracy(self):
        ...
        for element in data:
            ...
            for e in element:
                self.assertTrue(type(e[0] == str))
                self.assertTrue(type(e[1] == float))
    
    #@unittest.skip
    def test_data_accuracy_from_example(self):
        #set almost equal assertion decimal places
        decimal_places = 4
        network = generate_test_network()
        beliefs = network.predict_proba({'guest': 'A'})
        data = network.get_data(beliefs)
        correct_values = {'monty': 
                              ['monty', ['A', 0.0], ['B',0.5], ['C', 0.5]], 
                          'guest':
                              ['guest', ['A', 1]],
                          'prize':
                            ['prize', ['A', 1.0/3], ['B', 1.0/3], ['C', 1.0/3]]}
        
        for element in data:
            #store the correct values of the given state
            correct_v = correct_values[element[0]][1:]
            element_v = element[1:]
            #make sure the elements show up in the same order
            correct_v = sorted(correct_v, key=lambda x: x[0])
            element_v = sorted(element_v, key=lambda x: x[0])
            #print(f'Correct: {correct_v}')
            #print(f'Element: {element_v}')
            self.assertEqual(len(correct_v), len(element_v))
            for row in zip(correct_v, element_v):
                self.assertAlmostEqual(row[0][1], row[1][1], decimal_places)
            #print('===========================')
        
	#runs code to display the graph to check functionality of
	#the visual representationd
    #@unittest.skip
    def test_graph_visuals(self):
        network = generate_test_network()
        beliefs = network.predict_proba({'guest': 'A'})
